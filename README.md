...
Welcome to my Harvard CS50 final projects archive. This repository contains all of my solutions to the CS50 problem sets, refactored to run without the CS50 library. As a B.S. Computer Science graduate, I challenged myself with the more difficult assignment options in each problem set.

# How to Run
I set up a local development environment that closely resembles CS50’s online Codespace.
With this setup, you can simply download the project folder, navigate to the desired problem, and run make on the respective program—providing any required input as needed.

**Step 1: Install Windows Subsystem for Linux (WSL)**
- In Windows Powershell: "wsl --install" which will install Ubuntu and requires system restart.

**Step 2: Install Build Tools in Ubuntu**
- In Ubuntu terminal:
  - "sudo apt update"
  - "sudo apt install build-essential clang -y" (installs gcc, make, libraries, and clang compiler.)

**Step 3: Install WSL Extension in VSCode**
- Ctrl+Shift+X and search "WSL" and install.

**Step 4: Open the project folder in WSL via VSCode**
...
**credit**  
**Folder Contents: credit.c, Makefile**   
...
**scrabble**  
**Folder Contents: scrabble.c, Makefile**   
...
**readability**  
**Folder Contents: readability.c, Makefile**  
...
**substitution**  
**Folder Contents: substitution.c, Makefile**  
...
**plurality**  
**Folder Contents: plurality.c, Makefile**  
  ...
  **tideman**  
  **Folder Contents: tideman.c, Makefile**  
  ...
  ## Problem Set 4: Memory & File I/O

  **volume**  
  **Folder Contents: input.wav, Makefile, volume.c**  
  - This program manipulates a .WAV file.
  - Reads in and copies the .WAV header file and writes it to a new .wav file. (e.g. "./volume input.wav output.wav 0.2")
  ...
  **filter-more**  
  **Folder Contents: images(courtyard.bmp, stadium.bmp, tower.bmp, yard.bmp), bmp.h, filter.c, helper.c, helpers.h, Makefile**  
  ...
  **recover**  
  **Folder Contents: recover.c, card.raw, Makefile**  
  ...
  ## Problem Set 5: Data Structures & Pointers

  **inheritance**  
  **Folder Contents: inheritance.c, Makefile**  
  ...
  **speller**  
  **Folder Contents: dictionaries(large, small), keys(aca.txt, austen.txt, birdman.txt, burnett.txt, carroll.txt, cat.txt, consitution.txt, federalist.txt, frankenstein.txt, grimm.txt, her.txt, holmes.txt, homer.txt, lalaland.txt, mansfield.txt, pneumonoultramicroscopicsilicovolcanoconiosis.txt, revenant.txt, rinehart.txt, shakespeare.txt, stein.txt, stoker.txt, surgery.txt, tolstoy.txt, wells.txt, whittier.txt, wordsworth.txt, xueqin1.txt, xueqin2.txt), texts(aca.txt, austen.txt, birdman.txt, burnett.txt, carroll.txt, cat.txt, consitution.txt, federalist.txt, frankenstein.txt, grimm.txt, her.txt, holmes.txt, homer.txt, lalaland.txt, mansfield.txt, pneumonoultramicroscopicsilicovolcanoconiosis.txt, revenant.txt, rinehart.txt, shakespeare.txt, stein.txt, stoker.txt, surgery.txt, tolstoy.txt, wells.txt, whittier.txt, wordsworth.txt, xueqin1.txt, xueqin2.txt), dictionary.c, dictionary.h, Makefile, speller.c, speller50**
  - This program simulates a word checker.
  - It takes a dictionary and text file and checks if each word is in the dictionary. (e.g. ./speller dictionaries/large texts/grimm.txt).
  - It loads each dictionary word into a node and placed in the hash table based on the students hash function.
  - My hash function adds up to the first 6 chars and divides the sum by the number of buckets in the hash table. The remainder is the index.
  - Then adds the node to the hash table with a pointer to the next node. This creates the linked list.
